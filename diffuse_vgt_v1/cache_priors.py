...
def preview(scene,w,s,n,v,pool,save_image=True):
    ...
    if save_image: canvas.save(ROOT/'qc'/f'{scene}.png')
...
def main():
    ...
    if not 1<=args.limit<=24: raise ValueError('Invalid QC limit')
    if args.all:
        approval=json.loads((ROOT/'cache_approval.json').read_text())
        assert approval['status']=='PASS' and approval['scope']=='all_source_cache_only'
        for path,digest in approval['evidence'].items(): assert sha(ROOT/path)==digest
    torch.set_num_threads(4); torch.manual_seed(0); np.random.seed(0)
    manifest=json.loads((ROOT/'source_manifest.json').read_text())
    byid={r['scene_id']:r for r in manifest['sources']}
    scenes=list(byid) if args.all else manifest['qc_selection']['scenes'][:args.limit]
    pool=np.array([r['rgb_g1'] for r in manifest['allowed_endpoints']],np.float32)
    cache_dir=ROOT/('cache_all' if args.all else 'cache_qc')
    metadata_dir=ROOT/('cache_metadata' if args.all else 'qc')
    summary_path=ROOT/('cache_progress.json' if args.all else 'qc_summary.json')
    cache_dir.mkdir(exist_ok=True); metadata_dir.mkdir(exist_ok=True); (ROOT/'qc').mkdir(exist_ok=True)
    if any(cache_dir.glob('*.npz')): raise RuntimeError('Output cache not empty; preserve prior attempt rather than overwrite')
    ...
    save(ROOT/('prior_runtime_all.json' if args.all else 'prior_runtime.json'),dict(versions=versions,strict_load_all_models=True,
    ...
    for scene in scenes:
        ...
        valid=sg&nv
        np.savez_compressed(cache_dir/f'{scene}.npz',s_raw=sr,s0=s0,normal=n,valid_mask=valid,
                            shading_valid=sg,normal_valid=nv,risk_dark=(np.max(proxy,axis=2)<.01),
                            risk_sensor_saturation_unknown=np.ones((512,512),bool))
        vis=preview(scene,w,s0,n,valid,pool,save_image=not args.all)
        info=dict(scene_id=scene,input_sha256=row['sha256'],seconds=time.time()-t,
          ...
          preview=vis,cache_sha256=sha(cache_dir/f'{scene}.npz'))
        save(metadata_dir/f'{scene}.json',info); summary.append(info)
        save(summary_path,dict(status='IN_PROGRESS',count=len(summary),scenes=summary))
        print(json.dumps(dict(scene=scene,seconds=info['seconds'],invalid=float(1-valid.mean()))),flush=True)
    save(summary_path,dict(status='INFERENCE_COMPLETE',count=len(summary),seconds=time.time()-start,
         peak_gpu_bytes=torch.cuda.max_memory_allocated(),scenes=summary))
    if args.all:
        unusable=sum(r['preview']['common_valid_qc_patches']==0 for r in summary)/len(summary)
        if unusable>.2: raise RuntimeError('More than20percent sources technically unusable')
        save(ROOT/'cache_manifest.json',dict(schema_version=1,status='complete',
          source_manifest_sha256=sha(ROOT/'source_manifest.json'),priors_manifest_sha256=sha(ROOT/'priors_manifest.json'),
          cache_code_sha256=sha(Path(__file__)),normal_basis=np.eye(3).tolist(),unusable_source_fraction=unusable,
          files={r['scene_id']:dict(path=f"cache_all/{r['scene_id']}.npz",sha256=r['cache_sha256'],input_sha256=r['input_sha256']) for r in summary},
          seconds=time.time()-start,bytes=sum(p.stat().st_size for p in cache_dir.glob('*.npz'))))
